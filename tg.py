...
# ИСПРАВЛЕННОЕ ЛОГИРОВАНИЕ (без %s)
logging.basicConfig(
    level=logging.INFO,
...
# ---------- ДУЭЛИ: КНОПКИ "ПРИНЯТЬ/ОТКАЗАТЬСЯ" В ЧАТЕ, ВЫБОР - В ЛС ----------
...
# Словарь для хранения выборов (в памяти)
duel_choices = {}

def get_duel_winner(challenger_choice, opponent_choice):
    """
    0 - Пена, 1 - Массаж, 2 - Веник
    """
    if challenger_choice == opponent_choice:
        return 0
    if challenger_choice == 0 and opponent_choice == 1:  # Пена > Массаж
        return 1
    if challenger_choice == 1 and opponent_choice == 2:  # Массаж > Веник
        return 1
    if challenger_choice == 2 and opponent_choice == 0:  # Веник > Пена
        return 1
    return 2
...
# Обработка принятия/отказа в чате
...
async def duel_accept_decline(callback: CallbackQuery):
    ...
    if action == "decline":
        ...
        return
    # action == "accept"
    # Отправляем противнику в ЛС запрос выбора
    ...
    try:
        ...
        # Также уведомляем вызывающего, что выбор ожидается
        await bot.send_message(challenger_id,
                               ...
                               parse_mode="HTML")
        # Отправляем вызывающему клавиатуру выбора
...
# Обработка выбора предмета (через ЛС)
...
async def process_duel_choice(callback: CallbackQuery):
    ...
    # Если оба выбрали – определяем результат
...
# Таймер автоматического проигрыша (если не выбрали)
...
# Вызов дуэли (в чате появляются кнопки "Принять/Отказаться")
...
async def duel_by_reply(message: Message):
    ...
    # Таймер на 2 минуты для отмены вызова
    await asyncio.sleep(120)
